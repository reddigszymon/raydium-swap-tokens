...
async function callback(data: any, programId: string) {
  ...
  const accounts = info.transaction.transaction.message.accountKeys.map(
    (i: Buffer) => base58.encode(i)
  );
  for (const item of [
    ...info.transaction.transaction.message.instructions,
    ...info.transaction.meta.innerInstructions
    ...
    if (
      baseMintAccount === null ||
      quoteMintAccount === null ||
      marketAccount === null
    )
